...
ID="$(podman ps -q -f "name=linkassistant_container_runtime")"
if [ -n "$ID" ]; then
    echo "App already running; exec inside running container"
    podman exec "$ID" /opt/seopowersuite/seopowersuite/distr/specific/linkassistant/bin/linkassistant.sh "$@"
    exit 0
fi
...
podman run --rm \
       -w "/home/$USER" \
       --name "linkassistant_container_runtime" \
       --hostname="$NAME" \
       --user="$USER" \
       --shm-size=1G \
       --cap-drop=ALL \
       ...
       $FIXES \
       "$IMAGE_NAME" bash -c "/opt/seopowersuite/seopowersuite/distr/specific/linkassistant/bin/linkassistant.sh \"$@\"" bash "$@"
...
#       "$IMAGE_NAME" bash -c "cd linkassistant/Linkassistant; pipewire-pulse & ./Linkassistant --enable-features=UseOzonePlatform,WaylandWindowDecorations --ozone-platform=wayland" "$@"
