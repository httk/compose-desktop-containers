...
ID="$(podman ps -q -f "name=websiteauditor_container_runtime")"
if [ -n "$ID" ]; then
    echo "App already running; exec inside running container"
    podman exec "$ID" /opt/seopowersuite/seopowersuite/distr/specific/websiteauditor/bin/websiteauditor.sh "$@"
    exit 0
fi
...
podman run --rm \
       -w "/home/$USER" \
       --name "websiteauditor_container_runtime" \
       --hostname="$NAME" \
       --user="$USER" \
       --shm-size=1G \
       --cap-drop=ALL \
       ...
       $FIXES \
       "$IMAGE_NAME" bash -c "/opt/seopowersuite/seopowersuite/distr/specific/websiteauditor/bin/websiteauditor.sh \"$@\"" bash "$@"
...
#       "$IMAGE_NAME" bash -c "cd websiteauditor/Websiteauditor; pipewire-pulse & ./Websiteauditory --enable-features=UseOzonePlatform,WaylandWindowDecorations --ozone-platform=wayland" "$@"
