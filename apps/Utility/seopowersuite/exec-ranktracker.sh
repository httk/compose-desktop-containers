...
ID="$(podman ps -q -f "name=ranktracker_container_runtime")"
if [ -n "$ID" ]; then
    echo "App already running; exec inside running container"
    podman exec "$ID" /opt/seopowersuite/seopowersuite/distr/specific/ranktracker/bin/ranktracker.sh "$@"
    exit 0
fi
...
podman run --rm \
       -w "/home/$USER" \
       --name "ranktracker_container_runtime" \
       --hostname="$NAME" \
       --user="$USER" \
       --shm-size=1G \
       --cap-drop=ALL \
       ...
       $FIXES \
       "$IMAGE_NAME" bash -c "/opt/seopowersuite/seopowersuite/distr/specific/ranktracker/bin/ranktracker.sh \"$@\"" bash "$@"
...
#       "$IMAGE_NAME" bash -c "cd ranktracker/Ranktracker; pipewire-pulse & ./Ranktrackery --enable-features=UseOzonePlatform,WaylandWindowDecorations --ozone-platform=wayland" "$@"
