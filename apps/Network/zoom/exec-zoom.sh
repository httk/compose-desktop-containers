...
if [ -n "$ID" ]; then
    ...
    podman exec "$ID" bash -c 'cd zoom/zoom; LD_LIBRARY_PATH=/home/rar/zoom/zoom:/home/rar/zoom/zoom/Qt/lib exec ./zoom "$@"' "bash" "$@"
    exit 0
fi
...
podman run --rm \
       ...
       "$IMAGE_NAME" bash -c 'cd zoom/zoom; pipewire-pulse & LD_LIBRARY_PATH=/home/rar/zoom/zoom:/home/rar/zoom/zoom/Qt/lib exec ./zoom "$@"' "bash" "$@"
