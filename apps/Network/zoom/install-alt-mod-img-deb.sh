...
podman run \
       ...
       $FIXES \
       "$IMAGE_NAME" bash -c "mkdir -m 0700 -p /tmp/root/.gnupg && cd /tmp/root && gpg --import /files/Zoom.pubkey.pem && gpg --verify /files/zoom_amd64.deb && apt-get install -y /files/zoom_amd64.deb"
