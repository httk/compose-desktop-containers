...
sed -i 's/^xwayland=.*$/xwayland=false/g' "$IMAGE_DIR/home/.config/zoomus.conf"
...
podman run --rm \
       ...
       $VIDEO_DEVS \
       $FIXES \
       "$IMAGE_NAME" bash -c "cd discord/Discord; pipewire-pulse & xwayland-exec ./Discord" "$@"

#       "$IMAGE_NAME" bash -c "cd discord/Discord; pipewire-pulse & xwayland-exec ./Discord --enable-features=UseOzonePlatform,WaylandWindowDecorations --ozone-platform=wayland" "$@"
