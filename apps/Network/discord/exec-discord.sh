...
ID="$(podman ps -q -f "name=discord_container_runtime")"
if [ -n "$ID" ]; then
    echo "App already running; exec inside running container"
    podman exec "$ID" bash -c 'cd discord/Discord; ./Discord "$@"' "bash" "$@"
    exit 0
fi
...
podman run --rm \
       -w "/home/$USER" \
       --name "discord_container_runtime" \
       --hostname="$NAME" \
       --user="$USER" \
       --shm-size=512M \
       --cap-drop=ALL \
       ...
       $VIDEO_DEVS \
       $FIXES \
       "$IMAGE_NAME" bash -c 'cd discord/Discord; pipewire-pulse & ./Discord "$@"' "bash" "$@"
