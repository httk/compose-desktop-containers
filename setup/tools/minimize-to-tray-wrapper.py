...
class AppIndicatorWrapper:
    ...
    def launch_process(self):
        ...
        time.sleep(2)  # Allow the window time to appear
        ...
        while True:
            if self.process.poll() is not None:
                if not self.persist_on_exit:
                    Gtk.main_quit()  # Exit the wrapper if process quits
                    return
                else:
                    # Relaunch the process if persist_on_exit is True
                    self.launch_process()
            time.sleep(1)
    ...
    def on_quit(self, source):
        if self.process and self.is_program_running():
            try:
                ...
                self.process.wait(timeout=5)  # Wait for up to 5 seconds for the process to terminate
...
def main():
    ...
    app = AppIndicatorWrapper(cmd=args.cmd, name=args.name, icon=args.icon, persist_on_exit=args.persist_on_exit)
    Gtk.main()
