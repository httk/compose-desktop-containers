...
if [ -n "${PKGS_FILES// }" ]; then
    PKGS="RUN apt-get -y install $(cat $PKGS_FILES | sort -u | tr '\n' ' ')"
fi
if [ -n "${PKGS_NODEPS_FILES// }" ]; then
    PKGS_NODEPS="RUN apt-get -y --no-dependencies install $(cat $PKGS_NODEPS_FILES | sort -u | tr '\n' ' ')"
fi
if [ -n "${COMMANDS_FILES// }" ]; then
    COMMANDS="$(cat $COMMANDS_FILES)"
fi
if [ -n "${PRECOMMANDS_FILES// }" ]; then
    PRECOMMANDS="$(cat $PRECOMMANDS_FILES)"
fi
...
podman build -t app-container-img --label=wrap .
podman image prune -f --filter label=wrap

echo "app-container-img" > image.info
